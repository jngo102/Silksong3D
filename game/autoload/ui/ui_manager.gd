...
@onready var canvas: CanvasLayer = $UICanvas
...
func create_ui(ui: GDScript, start_closed: bool = false) -> BaseUI:
	...
	canvas.add_child(instantiated_ui)
...
func delete_ui(ui: GDScript) -> void:
	...
	if ui_to_delete:
		ui_to_delete.close()
		canvas.remove_child(ui_to_delete)
...
func open_ui(ui: GDScript) -> BaseUI:
	...
	ui_to_open.open()
	canvas.move_child(ui_to_open, canvas.get_child_count() - 1)
	return ui_to_open
